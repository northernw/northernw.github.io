...
2. dubbo架构
   ...
   <img src="/github/northernw.github.io/image/image-20200523162137463.png" alt="image-20200523162137463" style="zoom:50%;" />
...
3. Forking Cluster:并行调用多个服务器，只要一个成功即返回。通常用于实时性要求较高的读操作，但需要浪费更多的服务资源。通过过fork=n设置最大并行数。
   ...
   8. **远程调用细节**
      ...
      1. 服务提供者暴露一个服务的概要过程
         ...
         3. DubboProtocal的export方法，会启动NettyServer监听服务连接，并将服务注册到注册中心
      2. 服务消费者消费一个服务的概要过程
         1. ReferenceConfig的init()方法调用Protocal实现类的refer()方法生成Invoker实例
