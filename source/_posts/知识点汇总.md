...
10. https和http区别，有没有用过其他安全传输手段？
   ...
   ![image-20200411180438085](../../image/image-20200411180438085.png)
...
48. sizeCtl>0时，`(n << 1) - (n >>> 1) ` = 0.75n （表示阈值，超过阈值需要扩容）
...
57. ~~数据库表设计，索引~~ 【explain分析也看看】

  ​    

58. Redis的缓存淘汰策略、更新策略
...
60. 限流算法
    ...
    2. 滑动窗口法
       ...
       2. 可以用Redis有序集合实现..
       3. <img src="../../image/image-20200524153228157.png" alt="image-20200524153228157" style="zoom: 33%;" />
...
63. 秒杀场景设计，应付突然的爆发流量
    ...
    ![image-20200525191748562](../../image/image-20200525191748562.png)
...
66. 一致性哈希
    ...
    6. 在服务节点太少时，容易因节点分部不均匀而造成数据倾斜问题，可引入虚拟节点机制，即对每一个服务节点计算多个哈希，每个计算结果位置都放置一个此服务节点，称为虚拟节点。同时数据定位算法不变，只是多了一步虚拟节点到实际节点的映射。
       ...
       ![image-20200528142813487](../../image/image-20200528142813487.png)
...
69. ~~数据库原理~~，数据库中间件，索引优化

    1. [索引优化](https://juejin.im/post/5b68e3636fb9a04fd343ba99#heading-3)
